...
export default function PageHeader({
    ...
    return (
        <div className="mb-4 sm:mb-6 flex flex-col w-full space-y-3 sm:space-y-4">
            {/* Row 1: Title, Stats, Refresh, Add Button */}
            <div className="flex flex-col  gap-3">
                <div className="flex   justify-between sm:flex-row  items-center sm:justify-between gap-3">
                    ...
                    {filters.length > 0 && onFilterChange && (
                        <div className="flex gap-2 w-full sm:w-auto">
                            <CustomDropdown
                                ...
                                placeholder={filters.find(f => f.key === activeFilter)?.label || "Select Filter"}
                                onChange={(option: any) => onFilterChange(option.value)}
                            />
                        </div>
                    )}
